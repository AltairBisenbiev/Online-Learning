...
<ul class="nav nav-tabs" id="myTab" role="tablist">
	<?php
	require 'connect.php';
	$mod=mysqli_query($db,"SELECT * FROM test_java_variant");
...
<?php $mods=mysqli_query($db,"SELECT * FROM test_java_variant");
...
{$nomer_variant=$str['nomer_variant'];
 ...
 <form method="POST" action="test_cpp_result.php">
 	<input type="hidden" name="nomer_variant" value="<?php echo $nomer_variant;?>">
<?php

$data=mysqli_query($db, "SELECT * FROM test_java_question WHERE nomer_variant='$nomer_variant' ORDER BY nomer_question ASC");
...
$answers=mysqli_query($db,"SELECT * FROM test_java_answer WHERE nomer_variant='$nomer_variant' AND nomer_question='$nomer_question'");
