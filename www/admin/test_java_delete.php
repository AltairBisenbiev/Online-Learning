...
if(isset($_POST['delete']))
{
	$id=$_POST['fordel'];
	require '../connect.php';
	$data=mysqli_query($db,"SELECT * FROM test_java_question WHERE nomer_variant='$id'");
	while($r=mysqli_fetch_array($data))
  	{
		$nomer_question=$r['nomer_question'];
		$d=mysqli_query($db,"SELECT * FROM test_java_answer WHERE  nomer_variant='$id'");
		while($row=mysqli_fetch_array($d))
		{
			mysqli_query($db,"DELETE FROM test_java_answer WHERE  nomer_variant='$id'");
		}
    mysqli_query($db,"DELETE FROM test_java_question WHERE  nomer_variant='$id'");
	}
  mysqli_query($db,"DELETE FROM test_java_variant WHERE nomer_variant='$id'");
	mysqli_close($db);	
}
...
<table class="table table-striped table-bordered ">
  ...
  <tbody>
  	<?php
  	require '../connect.php';
  	$m=mysqli_query($db,"SELECT * FROM test_java_variant");
  ...
  <select class="test_java_delete form-control">
    <option>--Choose module--</option>
  <?php
require_once '../connect.php';
$data=mysqli_query($db,"SELECT * FROM test_java_variant");
while ($row=mysqli_fetch_array($data)) {
  echo "<option value='".$row['nomer_variant']."'>Module ".$row['nomer_variant']."</option>";
}
  ?>
</select><br>
  <span class="test_java_delete_ajax"></span>
