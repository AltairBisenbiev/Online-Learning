...
if(isset($_POST['delete']))
{
	$id=$_POST['fordel'];
	require '../connect.php';
	$data=mysqli_query($db,"SELECT * FROM test_cdb_question WHERE nomer_variant='$id'");
	while($r=mysqli_fetch_array($data))
  	{
		$nomer_question=$r['nomer_question'];
		$d=mysqli_query($db,"SELECT * FROM test_cdb_answer WHERE  nomer_variant='$id'");
		while($row=mysqli_fetch_array($d))
		{
			mysqli_query($db,"DELETE FROM test_cdb_answer WHERE  nomer_variant='$id'");
		}
    mysqli_query($db,"DELETE FROM test_cdb_question WHERE  nomer_variant='$id'");
	}
  mysqli_query($db,"DELETE FROM test_cdb_variant WHERE nomer_variant='$id'");
	mysqli_close($db);	
}
...
<table class="table table-striped table-bordered ">
  ...
  <tbody>
  	<?php
  	require '../connect.php';
  	$m=mysqli_query($db,"SELECT * FROM test_cdb_variant");
  ...
  <select class="test_cdb_delete form-control">
...
while ($row=mysqli_fetch_array($data)) {
  ...
  <span class="test_cdb_delete_ajax"></span>
