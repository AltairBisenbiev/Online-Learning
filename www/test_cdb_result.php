...
if(isset($_POST['answer']))
{
	...
	$d=mysqli_query($db,"SELECT * FROM test_cdb_question WHERE nomer_variant='$varnom' ORDER BY nomer_question ASC");
	$i=0;
	$s=0;
	$l=0;
	$str="";	
	while ($q=mysqli_fetch_array($d))
	{
		...
		$i++;
		$a=mysqli_query($db,"SELECT * FROM test_cdb_answer WHERE nomer_variant='$varnom' AND nomer_question='$nofq'");
		$str=$str."<td>";
...
<body id="result">
	...
	<div class="row">
		...
		<h1 class="text-danger">of <?php echo $ball;?> out of 100 points</h1>
		<a href="test_cdb.php" class="btn btn-primary">BACK</a>
	</div>
		<div class=" col-lg-4 col-sm-2"></div>
	</div>
