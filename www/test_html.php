...
<ul class="nav nav-tabs" id="myTab" role="tablist">
	<?php
	require 'connect.php';
	$mod=mysqli_query($db,"SELECT * FROM test_html_variant");
...
<?php $mods=mysqli_query($db,"SELECT * FROM test_html_variant");
...
{$nomer_variant=$str['nomer_variant'];
 ...
 <form method="POST" action="test_html_result.php">
 	<input type="hidden" name="nomer_variant" value="<?php echo $nomer_variant;?>">
<?php

$data=mysqli_query($db, "SELECT * FROM test_html_question WHERE nomer_variant='$nomer_variant' ORDER BY nomer_question ASC");
...
$answers=mysqli_query($db,"SELECT * FROM test_html_answer WHERE nomer_variant='$nomer_variant' AND nomer_question='$nomer_question'");
