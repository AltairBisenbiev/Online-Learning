...
if(isset($_POST['answer']))
{
	...
	$d=mysqli_query($db,"SELECT * FROM test_html_question WHERE nomer_variant='$varnom' ORDER BY nomer_question ASC");
	$i=0;
	$s=0;
	$l=0;
	$str="";	
	while ($q=mysqli_fetch_array($d))
	{
		...
		$i++;
		$a=mysqli_query($db,"SELECT * FROM test_html_answer WHERE nomer_variant='$varnom' AND nomer_question='$nofq'");
		$str=$str."<td>";
...
<body id="result">
	...
	<div class="row">
		...
		<a href="test_html.php" class="btn btn-primary">BACK</a>
	</div>
		<div class=" col-lg-4 col-sm-2"></div>
	</div>
